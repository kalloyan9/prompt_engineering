...
# Function to grade a student answer using Mistral via Ollama
def grade_with_ollama(question, model_answers, student_answer):
...
for i in before_learning_indexes:
    ...
    grade, feedback = grade_with_ollama(question, model_answers, student_answer)
...
for i in learn_indexes:
    ...
    student_answer = row['Student Answer']
    model_answers = [row[f"Model Answer {j}"] for j in range(1, 6)]
    _ = grade_with_ollama(question, model_answers, student_answer)
